        ...
        s8 *orient)
{
    ...
    ret = snprintf(property_name, sizeof(property_name), "inven,%s-orient",
            name);
...
int inv_mpu_parse_dt(struct device *dev, struct mpu_platform_data *pdata)
{
    ...
    if (IS_ERR(pdata->vdd_ana)) {
        ...
        dev_notice(dev, "regulator get failed vdd_ana rc=%d\n", rc);
    }
    pdata->vdd_i2c = regulator_get(dev, "inven,vcc_i2c");
    if (IS_ERR(pdata->vdd_i2c)) {
        rc = PTR_ERR(pdata->vdd_i2c);
        dev_notice(dev, "regulator get failed vcc_i2c rc=%d\n", rc);
    }

    /* wake gpio and delay */
    rc = of_get_named_gpio(np, "inven,wake-gpio", 0);
    if (!gpio_is_valid(rc)) {
        dev_notice(dev, "missing wake gpio\n");
    }
    pdata->wake_gpio = rc;
    rc = of_property_read_u32_array(np, "inven,wake-delay-us", wake_up_delays,
            2);
    if (rc) {
        dev_notice(dev, "missing wake delays\n");
    }
    pdata->wake_delay_min = wake_up_delays[0];
    pdata->wake_delay_max = wake_up_delays[1];

    /* normal data interrupt */
    rc = of_get_named_gpio(np, "inven,irq-gpio", 0);
    if (!gpio_is_valid(rc)) {
        dev_notice(dev, "missing irq gpio\n");
    }
    pdata->irq_gpio = rc;
