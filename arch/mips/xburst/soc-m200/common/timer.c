/* arch/mips/xburst/soc-m200/common/timer.c
 * Setting up the clock for Ingenic SoC M200.
 *
...
#define CSRDIV(x)      ({int n = 0;int d = x; \
		while(d){ d >>= 2;n++;};(n-1) << 3;})
...
{
	return ((cycle_t)jz_get_cycles(0) * tmr_src.cs.mult) >>
		tmr_src.cs.shift;
}
...
{
	unsigned long flags;
	struct jz_timerevent *evt_dev =
		container_of(clk_evt_dev,struct jz_timerevent,clkevt);
...
static void jz_set_mode(enum clock_event_mode mode,
			struct clock_event_device *clkevt)
{
	struct jz_timerevent *evt_dev = container_of(clkevt,
			struct jz_timerevent, clkevt);
	unsigned long flags;
...
void __cpuinit jzcpu_timer_setup(void)
{
	...
	tcu_writel(TCU_TMSR, (1 << CLKEVENT_CH) |
			(1 << (CLKEVENT_CH + 16)));
